...
PRODUCTS_JSON_FILE="../week2/conf/bbuy_products.json"
QUERIES_JSON_FILE="../conf/bbuy_queries.json"
DATASETS_DIR="../workspace/datasets"
PYTHON_LOC="./utilities"
...
if [ "$ANNOTATE" != "--annotate" ]; then
  ...
  if [ -f $PRODUCTS_JSON_FILE ]; then
    ...
    curl -k -X PUT -u admin  "http://localhost:9200/bbuy_products" -H 'Content-Type: application/json' -d "@$PRODUCTS_JSON_FILE"
    if [ $? -ne 0 ] ; then
...
#    curl -k -X PUT -u admin  "http://localhost:9200/bbuy_queries" -H 'Content-Type: application/json' -d "@$QUERIES_JSON_FILE"
#    if [ $? -ne 0 ] ; then
...
if [ "$ANNOTATE" == "--annotate" ]; then
  ...
  if [ -f $ANNOTATIONS_JSON_FILE ]; then
    ...
    curl -k -X PUT -u admin  "http://localhost:9200/bbuy_annotations" -H 'Content-Type: application/json' -d "@$ANNOTATIONS_JSON_FILE"
    if [ $? -ne 0 ] ; then
