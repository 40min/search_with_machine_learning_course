...
curl -k -X DELETE -u admin  "http://localhost:9200/bbuy_products"
if [ $? -ne 0 ] ; then
...
curl -k -X DELETE -u admin  "http://localhost:9200/bbuy_queries"
if [ $? -ne 0 ] ; then
...
curl -k -X DELETE -u admin  "http://localhost:9200/bbuy_annotations"
